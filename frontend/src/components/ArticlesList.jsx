...
export const ArticlesList = ({ data, loading, error }) => {
  ...
  return (
    <>
      {loading ? (
        <Skeleton type="articleList" />
      ) : error ? (
        <div className="border-b pb-16 mb-16 w-[80%] md:w-[75%] mx-auto">
          ...
          <div className="" key={article._id}>
            <div className="text-center w-[80%] md:w-[70%] mx-auto flex flex-col items-center gap-3">
              ...
              <div className="flex italic items-center gap-3 mb-2 md:text-sm">
                <p className="text-[#767676] text-sm sm:text-xs">
              ...
              <div className="flex text-[#333333] self-start gap-2 items-center">
                ...
                <div className="p-3 text-[#333333] hover:text-[#edb48b] transition duration-200 ease">
                  <span className="h-full flex justify-center items-center text-sm italic">
                    {article.access === "Free" ? (
                      ""
                    ) : (article.access === "Basic" && sub === "Premium") ||
                      sub === "Basic" ? (
                      ""
                    ) : article.access === "Premium" && sub === "Premium" ? (
                      ""
                    ) : (
                      <LockIcon style={{ fontSize: "14px" }} />
                    )}
                    {article.access === "Free" || sub === "Premium"
                      ? ""
                      : article.access}
                  </span>
                </div>
